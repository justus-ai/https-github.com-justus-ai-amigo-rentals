...
if (propertyCount === 0) {
  ...
  const insertProperty = db.prepare(`
    INSERT INTO properties (type, title, location, lat, lng, price, bedrooms, bathrooms, area, land_size, purchase_price, image, images, description, available, created_at, updated_at)
    VALUES (@type, @title, @location, @lat, @lng, @price, @bedrooms, @bathrooms, @area, @land_size, @purchase_price, @image, @images, @description, @available, @created_at, @updated_at)
  `);
...
app.get('/api/properties', (_req, res) => {
  const rows = db
  .prepare('SELECT id, type, title, location, lat, lng, price, bedrooms, bathrooms, area, land_size, purchase_price, image, images, description, available FROM properties ORDER BY id DESC')
    .all();
...
app.post('/api/properties', requireAuth, (req, res) => {
  ...
  const created = db
    .prepare('SELECT id, type, title, location, lat, lng, price, bedrooms, bathrooms, area, land_size, purchase_price, image, images, description, available FROM properties WHERE id = ?')
